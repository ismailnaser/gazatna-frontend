    ...
    <div className="space-y-4">
      <Card className="space-y-4 p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-xl border border-neutral-200 bg-p-cream/30 px-4 py-3">
            <p className="text-xs text-p-black/75">الحالة</p>
            <div className="mt-1">
              <Badge variant="default">مؤرشفة</Badge>
            </div>
          </div>
          <div className="rounded-xl border border-neutral-200 bg-p-cream/30 px-4 py-3">
            <p className="text-xs text-p-black/75">عدد الفصول</p>
            <p className="mt-1 text-lg font-bold text-p-black">{selectedYear.terms.length}</p>
          </div>
          {selectedYear.createdAt ? (
            <div className="rounded-xl border border-neutral-200 bg-p-cream/30 px-4 py-3">
              <p className="text-xs text-p-black/75">تاريخ الإنشاء</p>
      ...
      <Card className="overflow-hidden p-0">
        ...
        {selectedYear.terms.length === 0 ? (
          <p className="px-4 py-6 text-sm text-neutral-700">لا توجد فصول مسجّلة لهذه السنة.</p>
        ) : (
            ...
            {selectedYear.terms.map((term) => (
              ...
              >
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-p-black/75">
        ...
        {loadingCertificate ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3">
              <p className="text-xs text-p-black/75">دورة الإصدار</p>
              <p className="mt-1 font-semibold text-p-black">
                {certificateScopeLabels[certificateConfig.issuanceScope]}
              </p>
            </div>
            <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3">
              <p className="text-xs text-p-black/75">حالة النشر</p>
              <div className="mt-1">
            ...
            {certificateConfig.isPublished && certificateConfig.publishedAt ? (
              <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3 sm:col-span-2">
                <p className="text-xs text-p-black/75">تاريخ النشر</p>
                <p className="mt-1 font-semibold text-p-black">
          ...
          <p className="text-sm text-neutral-700">لا توجد إعدادات شهادات محفوظة لهذه السنة.</p>
        )}
      </Card>
    </div>
  );
