...
export function ImagePreviewModal({ open, src, title, onClose }: ImagePreviewModalProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={title ?? "معاينة الصورة"}
      onClick={onClose}
    >
      <div
      ...
      >
        <header className="flex items-center justify-between gap-3 border-b border-neutral-100 px-4 py-3">
          <p className="truncate text-sm font-semibold text-p-black">{title ?? "معاينة الصورة"}</p>
          <button
            type="button"
            onClick={onClose}
            aria-label="إغلاق"
