...
export function buildStudentSchedulePrintHtml(schedule: Schedule) {
  const lines = [SCHEDULE_TYPE_LABELS[schedule.scheduleType]];
  if (schedule.classLabels.length > 0) {
    lines.push(`الشعبة: ${schedule.classLabels.join(" · ")}`);
  }
  return {
    title: schedule.name,
    bodyHtml: `
      <div style="font-family:Tahoma,Arial,sans-serif;">
        <h1 style="margin:0 0 8px;font-size:20px;">${escapeHtml(schedule.name)}</h1>
        ${lines.map((line) => `<p style="margin:0 0 4px;font-size:13px;color:#555;">${escapeHtml(line)}</p>`).join("")}
        <div style="margin-top:16px;">${buildStudentGridPdfHtml(schedule)}</div>
      </div>
    `,
  };
}

export function buildTeacherSchedulePrintHtml(rows: TeacherScheduleRow[], title = "جدول حصصي الأسبوعي") {
  const displayRows = sortTeacherScheduleRows(rows);
  const headers = ["اليوم", "موعد الحصة", "الفصل والشعبة", "المادة"];
  const rowsHtml =
    displayRows.length === 0
  ...
  return {
    ...
    bodyHtml: `
      <div style="font-family:Tahoma,Arial,sans-serif;">
        ...
        <p style="margin:0 0 4px;font-size:13px;color:#555;">جدول حصصي الأسبوعي</p>
